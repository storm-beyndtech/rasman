...
const DashboardEmptyState: React.FC<DashboardEmptyStateProps> = ({ type, searchQuery }) => {
  const emptyStates = {
    'no-purchases': {
      ...
      description: 'Start building your conscious reggae collection with Rasman\'s latest tracks and timeless classics.',
      action: (
      ...
      action: (
        <div className="flex flex-col sm:flex-row gap-4 justify-center">
          <button
            ...
            className="bg-reggae-green text-white px-8 py-3 rounded-xl font-semibold hover:bg-green-600 transition-all duration-300"
          >
            Clear Filters
          </button>
          <Link
            href="/songs"
            className="bg-black/20 backdrop-blur-md text-white px-8 py-3 rounded-xl font-semibold hover:bg-black/30 transition-all duration-300 border border-white/20"
          >
            Browse All Music
          </Link>
        </div>
