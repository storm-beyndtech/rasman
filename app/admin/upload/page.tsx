...
const FileUpload: React.FC<{
  ...
  <div>
    <label className="block text-sm font-medium text-gray-300 mb-3">{type === "audio" ? "Audio File *" : "Cover Art *"}</label>
    <div
      className={`border-2 border-dashed border-gray-700/50 rounded-xl p-8 text-center transition-all duration-300 ${
        uploading ? "opacity-50 cursor-not-allowed" : "hover:border-reggae-green/50 cursor-pointer hover:bg-black/20"
      }`}
    >
      <input
        type="file"
      ...
      <label htmlFor={`${type}-upload`} className={uploading ? "cursor-not-allowed" : "cursor-pointer"}>
        ...
        {file ? (
          ...
          <>
            <p className="text-sm text-gray-300 mb-1">Click to upload {type === "audio" ? "audio file" : "cover art"}</p>
            <p className="text-xs text-gray-500">{type === "audio" ? "MP3, WAV, FLAC up to 50MB" : "JPG, PNG up to 5MB"}</p>
          </>
        )}
      </label>
    </div>
      ...
      >
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <FileUpload type="audio" file={state.songForm.audioFile} handleFileSelect={handleFileSelect} uploading={state.uploading} />
            <FileUpload type="cover" file={state.songForm.coverFile} handleFileSelect={handleFileSelect} uploading={state.uploading} />
          </div>
