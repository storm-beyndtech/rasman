...
async function handleSuccessfulPayment(data: any) {
	try {
		...
		const [item, userProfile] = await Promise.all([
			itemType === "song" ? Song.findById(itemId) : Album.findById(itemId).populate("songIds"),
...
async function handleFailedPayment(data: any) {
	try {
		...
		await Purchase.findOneAndUpdate({ paymentId: reference, _id: purchaseId }, { status: "failed" });
